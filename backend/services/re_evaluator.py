...
class ReEvaluator:
    ...
    def calculate_score_from_details(self, details: list, partial_credit: bool = True) -> float:
        ...
        for detail in details:
            ...
            if detail.get('is_correct') is True: total_score += 1.0
            elif partial_credit and 'partial_credit' in detail:
                try:
                    credit = float(detail.get('partial_credit', 0))
                    total_score += max(0.0, min(1.0, credit))
                except Exception: pass
        return round((total_score / total_questions) * 100.0, 2) if total_questions > 0 else 0.0
    ...
    async def re_evaluate_file(self, file_path: str, title: str, description: str, file_id: Optional[str] = None, db: Optional[Session] = None, current_user: Optional["User"] = None) -> Dict:
        try:
            ...
            eval_tasks = []
            for qa in qa_pairs:
                eval_tasks.append(self.gemini_service.evaluate_one_qa(description, qa.get('question', ''), qa.get('answer') or qa.get('student_answer', '')))
            ...
            for res in eval_results:
                ...
                details.append(res.get("response").model_dump())
